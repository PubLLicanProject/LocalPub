...
DB_FILE = get_base_path()+'/'+'pm_ta.db'
...
def get_keyword(keywords, db_file = DB_FILE, keytype = None):
    ...
    cur = conn.cursor()
    cur.execute('PRAGMA cache_size = -30000000;')  # Negative value means the cache size is in pages
...
def evaluate_postfix(words, dbfile):
    ...
    for token in expression:
        if token.lower() in {"and", "or", "not"}:
            if token.lower() == "not":
                ...
                operand = stack.pop()
                if not stack:
                    raise ValueError("Cannot apply 'NOT' without a keyword to negate.")
    ...
    if not positive_keywords:
        raise ValueError("The expression must contain at least one keyword to evaluate.")
