...
BLOCK_SIZE = "auto"
log = {"dldict":{},"existdict":{},"corrupt":{},"errdict":{}}


def updateLog(code, fileName):
    """ Used for crating the json output for pipeline"""
    match code:
        case 100: log["dldict"].update({fileName: "Sucessfully downloaded"})
        case 200: log["existdict"].update({fileName: "Already exists. No corruption"})
        case 300: log["corrupt"].update({fileName: "File is corrupted"})
        case 400: log["errdict"].update({fileName: {"Error": "Failed to download the file"}})
        case 401: log["errdict"].update({fileName: {"Error": "File was not found in user specified path"}})
        case 402: log["errdict"].update({fileName: {"Error": "User Interrupted process."}})
        case 404: log["errdict"].update({fileName: {"Error": "Download failed. Connection error."}})
...
if __name__ == "__main__":
    ...
    parser.add_argument('-l', action='store_true', help="list the downloadable files. Outputs list in a json")
    parser.add_argument('-p', metavar=("[directory]"), type=str, nargs=1, help="for use in a pipeline. download all PMC tar files and prints a json in stdout.")
    parser.add_argument('-d', metavar=("[file]","[path]"), type=str, nargs=2, help="download a file from the link")
    ...
    match op:
        ...
        case "m": # calculate md5sum of a file and compare with md5 passed as param
            ...
            # if md5sum(args.m[0], args.m[1]):
                # print(f"{args.m[1]} not corrupted.")
            # else:
                # print(f"{args.m[1]} is corrupted")
